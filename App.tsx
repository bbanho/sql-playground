    ...
    <div className="flex flex-col h-screen overflow-hidden bg-ice-200 dark:bg-slate-950 font-sans">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        >
          <MissionSidebar 
            missions={missions} 
            currentId={currentMission?.id} 
            onSelect={(id) => setCurrentMission(missions.find(m => m.id === id)!)} 
            fontSize={fontSize} 
        ...
        <main className="flex-1 flex flex-col min-w-0 relative bg-ice-200 dark:bg-slate-950">
          {/* Mission Header */}
          <div className="bg-white dark:bg-slate-900 border-b border-ice-300 dark:border-slate-800 flex items-center justify-between shadow-sm z-20 px-4 py-3 h-14">
             <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="relative group">
                  <select 
                    ...
                    className="appearance-none bg-transparent text-slate-700 dark:text-slate-300 font-bold text-xs uppercase tracking-wide cursor-pointer focus:outline-none pr-4"
                  >
                ...
                {currentMission && (
                  <div className="truncate">
                    <span className="text-blue-600 dark:text-blue-500 font-bold mr-2 text-sm">#{currentMission.id}</span>
                    <span className="text-slate-700 dark:text-slate-200 font-semibold text-sm truncate">{currentMission.title}</span>
                  </div>
                )}
             </div>
